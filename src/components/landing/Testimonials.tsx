      ...
      <div className="max-w-6xl mx-auto">
        ...
        loop: true
          ...
          <CarouselContent className="-ml-1">
            {testimonials.map(testimonial => <CarouselItem key={testimonial.id} className="pl-1 md:basis-1/2 lg:basis-1/3">
                <div className="bg-slate-700/90 p-6 rounded-lg h-full flex flex-col shadow-lg border border-slate-600 mx-[10px]">
                  <div className="flex mb-4">
                    {Array.from({
                  length: 5
                }).map((_, i) => <Star key={i} className={`w-5 h-5 ${i < testimonial.rating ? "text-[#C53030] fill-[#C53030]" : "text-gray-400"}`} />)}
                  </div>

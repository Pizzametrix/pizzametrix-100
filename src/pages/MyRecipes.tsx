...
export default function MyRecipes() {
    ...
    const fetchRecipes = async () => {
      try {
        ...
        const {
          data,
          error
        } = await supabase.from('recettes').select(`
            *,
            photos (
              url
            )
  ...
  return <div className="min-h-screen flex flex-col md:flex-row bg-slate">
      <Sidebar />
      <main className="flex-1 p-4 pb-24 md:p-8 md:pb-24 mt-16 md:mt-0 py-[22px]">
        <div className="max-w-2xl mx-auto space-y-4">
          {loading ? <p className="text-cream text-center">Chargement de vos recettes...</p> : recipes.length === 0 ? <p className="text-cream text-center">Vous n'avez pas encore de recettes enregistrées.</p> : recipes.map(recipe => <RecipeCard key={recipe.id} recipe={recipe} />)}
        </div>
      </main>
    </div>;
}